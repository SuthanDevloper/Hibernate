...
public class Test {
	...
	public static void main(String[] args) {
		...
		//Here we take employee table and add to Employee entity 
		
			SQLQuery sql_query = session.createSQLQuery("select * from emp1 where esal < :sal_Range");
			sql_query.setFloat("sal_Range",10000);
			
				SQLQuery sql_query1 = session.createSQLQuery("select * from emp1 where esal >= :min and  esal <= :max ");
				sql_query1.setFloat("min",4000);
				sql_query1.setFloat("max",10000); 
				sql_query1.addEntity(Employee.class);
				
		List<Employee> list =sql_query1.list();
		...
		SQLQuery sql_query2 = session.createSQLQuery("select * from emp1 where esal >= ? and  esal <= :max "); 
		sql_query2.setFloat(0,4000);
		sql_query2.setFloat("max",10000);
		sql_query2.addEntity("com.suthan.pojo.Employee"); 
	//--------------------------------------------------------------------	
		//NAMED QUERY ALSO
		/*
